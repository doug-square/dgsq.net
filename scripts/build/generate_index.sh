...
EOF
        ...
        if [ $current_page -gt 1 ]; then
            ...
            echo "    <a href=\"$(fix_url "$prev_url")\" class=\"prev\">&laquo; ${MSG_NEWER_POSTS:-Newer}</a>" >> "$output_file"
        fi
        ...
        if [ $current_page -lt $total_pages ]; then
            ...
            echo "    <a href=\"$(fix_url "/page/$next_page/")\" class=\"next\">${MSG_OLDER_POSTS:-Older} &raquo;</a>" >> "$output_file"
        fi
